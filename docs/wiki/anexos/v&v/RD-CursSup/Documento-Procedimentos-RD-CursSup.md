# Documento de Procedimentos - (RD-CursoSup)
...
### 3.1 Procedimentos
- O diagrama de entidade e relacionamento definido no documento de Requisitos do Software SempreUFG e as classes que implementam os teste referente ao RD-CursoSup são as bases para criar os casos de testes.
- O casos de testes foram aplicados nas classes geradas na implementação dos testes do RD-GestImport, disponibilizadas no respositório do projeto, realizando a execução dos testes implementados e coletando seus resultados, e analizando o código de teste.
